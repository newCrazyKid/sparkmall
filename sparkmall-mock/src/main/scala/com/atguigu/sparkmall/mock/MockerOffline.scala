...
import com.atguigu.sparkmall.mock.util.{RandomDate, RandomNum, RandomOptions}
...
object MockerOffline {
    ...
    def main(args: Array[String]): Unit = {
        //!!!不加这行，会以Windows的用户名去登录hdfs
        System.setProperty("HADOOP_USER_NAME", "lin10")
    ...
    def insertHive(sparkSession: SparkSession, tableName: String, dataFrame: DataFrame)={
    ...
    def mockUserAction()={
        ...
        val startDate = new SimpleDateFormat("yyyy-MM-dd").parse("2019-02-01")
        val endDate = new SimpleDateFormat("yyyy-MM-dd").parse("2019-02-02")
        ...
        for(i <- 1 to sessionNum){
            ...
            while(!isQuit){
                ...
                if(action == "quit"){
                    ...
                    val userVisitAction = UserVisitAction(actionDateString,
                        userId.toLong,
                        sessionId,
    ...
    def mockProductInfo()={
        ...
        for(i <- 1 to productNum){
            val productInfo = ProductInfo(i, "商品_" + i, productExRandomOpt.getRandomOpt())
            rows += productInfo
        }
        rows.toList
    }
