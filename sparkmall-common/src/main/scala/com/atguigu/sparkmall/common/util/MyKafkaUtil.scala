...
object MyKafkaUtil {
    ...
    val kafkaParam = Map(
        ...
        //如果没有初始化偏移量或者当前偏移量不存在任何服务器上，可以使用这个配置属性，latest自动重置偏移量为最新的偏移量
        "auto.offset.reset" -> "latest",
        //如果是true，则这个消费者的偏移量会在后台自动提交，但是kafka当即容易丢失数据
    ...
    /**
      ...
      * LocationStrategies.PreferConsistent：时序在所有Executor之间分配分区
      * ConsumerStrategies：选择如何在Driver和Executor上创建和配置consumer
